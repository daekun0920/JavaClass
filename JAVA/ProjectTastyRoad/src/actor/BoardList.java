...
public class BoardList {
	...
	public static void pageViewExt(ArrayList<String> list) { // 순수 페이징 (비회원 검색 페이징)
		pagingExt(list);
	}
		...
		while(flag) {
			try {
				...
				if (list.get(0) instanceof MemberTable) {
					...
					System.out.println("[번호]\t[작성시간]\t[아이디]\t\t[내용]");
				}
				
				for(int i = start - (pageNum * 10); i > start - (pageNum * 10 + 10); i--) {
					if (list.get(0) instanceof Board) {
						
					System.out.printf("%-4s\t%-8s\t%-20s\t%s\n"	, ((Board) list.get(i)).getBoardIndex()
					...
					if(input.equals("종료")) {
						...
						if (list.get(0) instanceof MemberTable) {
							...
							if (Business.m.getMemberType().equals("회원")) {
								...
								Admin.adminScreen();
							} else if (Business.m.getMemberType().equals("점주")) {
								Business.ownerMenu();
							}
		...
		while(flag) {
			try {
				...
				for(int i = start - (pageNum * 10); i > start - (pageNum * 10 + 10); i--) {
					...
					System.out.printf("%-30.18s \t\t\t\t%-30.12s \t%-18.10s %-30.24s %-5s %-40.30s %-40s"
									 , temp[0]
									 , temp[1]
									 , temp[2]
									 , temp[3]
