...
//	
	...
	deleteFiles(dir, dirCount);
	//deleteFolders(dir, dirCount);
	
	System.out.printf("폴더 %d개와 파일 %d개 삭제를 완료했습니다.", dirCount[1], dirCount[0]);
	...
	*/
	...
	...
		*/
//	String path1 = "D:\\Class\\Java\\디렉토리 문제\\동일 파일\\Music_1";
//	String path2 = "D:\\Class\\Java\\디렉토리 문제\\동일 파일\\Music_1";
//	
...
//	String path = "D:\\Class\\Java\\디렉토리 문제\\크기 정렬";
//	File dir = new File(path);
//	
//	fileCheck(allFiles, dir);
//	compareFiles(allFiles);
//	
	...
	*/
	...
	private static void compareFiles(ArrayList<File> allFiles) {
	...
	private static void fileCheck(ArrayList<File> allFiles, File dir) {
		if (dir.exists()) {
			File[] files = dir.listFiles();
			for (File file : files) {
				if (file.isFile()) {
	...
	private static void arrangeFiles(String path) {
		File dir = new File(path);
		...
		for (File file : files) {
			...
			if (file.isFile()) {
				index = file.getName().indexOf("_");
				name = file.getName().substring(0, index); // 이름만 분리 
				year = file.getName().substring(index + 1, index + 5); // 연도만 분리 
			
			
				String yearPath = String.format("D:\\Class\\Java\\디렉토리 문제\\직원\\%s\\%s", name, year);
				File yearDir = new File(yearPath);
				
				String newPath = String.format("D:\\Class\\Java\\디렉토리 문제\\직원\\%s", name);
				File newDir = new File(newPath);
				
				if (!newDir.exists() || !yearDir.exists()) {
					newDir.mkdir();
					yearDir.mkdir();
				} 
				
				String fullName = yearDir + "\\" + file.getName();
				File fullDir = new File(fullName);
				
				
				file.renameTo(fullDir);
			} // if 
		
		} // for 
		System.out.println("파일 생성과 이동이 완료 되었습니다.");
	} // arrangeFiles(#7)
	...
	private static void deleteFiles(File dir, int[] dirCount) {
		File[] files = dir.listFiles();
...
//	private static void deleteFolders(File dir, int[] dirCount) {
//		File[] files = dir.listFiles();
//		
//		if (dir.exists() ) {
