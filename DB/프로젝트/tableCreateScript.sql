...
(
  ...
  basic_course_name VARCHAR2(300) NOT NULL
);
...
(
  ...
  basic_subject_name VARCHAR2(300) NOT NULL
);
...
(
  ...
  basic_book_name VARCHAR2(300) NOT NULL,
...
(
  ...
  basic_book_seq NUMBER REFERENCES tbl_basic_book (basic_book_seq) NOT NULL
);
...
(
  grading_standard_seq NUMBER PRIMARY KEY,
...
(
  student_attendance_seq NUMBER PRIMARY KEY,
...
(
    student_attendance_seq NUMBER REFERENCES tbl_student_attendance (student_attendance_seq) NOT NULL,
...
(
  ...
  course_review VARCHAR2(1000) NOT NULL
);
...
(
  student_grade_seq NUMBER PRIMARY KEY,
...
-- 참조키 테이블 --
CREATE TABLE tbl_key_subject_course
(
...
CREATE TABLE tbl_key_student_quit_course
(
    course_seq NUMBER REFERENCES tbl_course (course_seq) NOT NULL,
    student_quit_seq NUMBER REFERENCES tbl_student_quit (student_quit_seq) NOT NULL
);

CREATE TABLE tbl_key_enrollment_course
(
    course_seq NUMBER REFERENCES tbl_course (course_seq) NOT NULL,
    student_enrollment_seq NUMBER REFERENCES tbl_student_enrollment (student_enrollment_seq) NOT NULL
);

CREATE TABLE tbl_key_complete_course
(
    course_seq NUMBER REFERENCES tbl_course (course_seq) NOT NULL,
    student_complete_seq NUMBER REFERENCES tbl_student_complete (student_complete_seq) NOT NULL

);

CREATE TABLE tbl_key_teacher_subject
(
    subject_seq NUMBER REFERENCES tbl_subject (subject_seq) NOT NULL,
    detail_teacher_seq NUMBER REFERENCES tbl_detail_teacher (detail_teacher_seq) NOT NULL
);

CREATE TABLE tbl_key_standard_subject
(
    subject_seq NUMBER REFERENCES tbl_subject (subject_seq) NOT NULL,
    grading_standard_seq NUMBER REFERENCES tbl_grading_standard (grading_standard_seq) NOT NULL

);

CREATE TABLE tbl_key_grade_subject
(
    subject_seq NUMBER REFERENCES tbl_subject (subject_seq) NOT NULL,
    student_grade_seq NUMBER REFERENCES tbl_student_grade (student_grade_seq) NOT NULL
    
);

CREATE TABLE tbl_key_student_enrollment 
(
    student_enrollment_seq NUMBER REFERENCES tbl_student_enrollment (student_enrollment_seq) NOT NULL,
    detail_student_seq NUMBER REFERENCES tbl_detail_student (detail_student_seq) NOT NULL
);
